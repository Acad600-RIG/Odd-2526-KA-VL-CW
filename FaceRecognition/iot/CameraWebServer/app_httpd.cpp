...
// === External AI server config ===
static const char *AI_HOST = "192.168.1.28"; // TODO: set to your PC LAN IP
static const int AI_PORT = 8000;
static TaskHandle_t auto_verify_task_handle = NULL;
...
// Send current frame to FastAPI /verify-face as multipart JPEG
static esp_err_t post_verify(camera_fb_t *fb)
{
    const char *boundary = "----esp32cam-boundary";
    char url[96];
    snprintf(url, sizeof(url), "http://%s:%d/verify-face", AI_HOST, AI_PORT);
    ...
    int content_len = head_len + fb->len + tail_len;
    ...
    {
        log_i("AI verify success: %s", resp);
        return ESP_OK;
    }

    log_w("AI verify failed, status %d, body: %s", status, resp);
    return ESP_FAIL;
}

// Lightweight face presence check before auto-posting
static bool has_face(camera_fb_t *fb)
{
#if CONFIG_ESP_FACE_DETECT_ENABLED
    bool detected = false;
    ...
    else
    {
        ...
        if (!out_buf)
        {
            return true; // fall back to sending if memory is low
        }
        ...
        {
            free(out_buf);
            return true; // fall back to sending if conversion fails
        }
        ...
        HumanFaceDetectMSR01 s1(0.1F, 0.5F, 10, 0.2F);
        HumanFaceDetectMNP01 s2(0.5F, 0.3F, 5);
        std::list<dl::detect::result_t> &candidates = s1.infer((uint8_t *)out_buf, {height, width, 3});
        std::list<dl::detect::result_t> &results = s2.infer((uint8_t *)out_buf, {height, width, 3}, candidates);
#else
        HumanFaceDetectMSR01 s1(0.3F, 0.5F, 10, 0.2F);
        std::list<dl::detect::result_t> &results = s1.infer((uint8_t *)out_buf, {height, width, 3});
#endif
...
// Background task: every 5 seconds check for a face and forward to FastAPI
static void auto_verify_task(void *param)
{
    ...
    while (true)
    {
        ...
        if (!fb)
        {
            log_w("auto_verify: camera capture failed");
            vTaskDelay(delay_ticks);
            continue;
        }

        bool detected = has_face(fb);
        if (detected)
        {
            esp_err_t ok = post_verify(fb);
            log_i("auto_verify: sent frame, result %s", ok == ESP_OK ? "OK" : "FAIL");
        }
        else
        {
            log_i("auto_verify: skipped, no face");
        }
...
// Capture one frame, send to FastAPI verify, return JSON result
static esp_err_t verify_handler(httpd_req_t *req)
{
    ...
    if (ok == ESP_OK)
    {
        const char *body = "{\"status\":\"forwarded\"}";
...
void startCameraServer()
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        if (auto_verify_task_handle == NULL)
        {
#if CONFIG_ESP_FACE_DETECT_ENABLED
            log_i("Auto verify task enabled (face-gated) every 5s");
#else
            log_i("Auto verify task enabled (no face gating) every 5s");
#endif
            xTaskCreatePinnedToCore(auto_verify_task, "auto_verify_task", 8192, NULL, 4, &auto_verify_task_handle, 1);
        }
    }
